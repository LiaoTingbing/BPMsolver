...
sp_cx_mat dxdxfunc(const cx_vec& p, const cx_vec& q, const cx_vec& r, int nx, int ny, double dx, double dy, DiagVector& diagv)
{
	int ng = nx * ny;
	cx_vec a = p % join_cols(cx_vec(1), r.rows(0, ng - 2))
		/ (join_cols(cx_vec(1), q.rows(0, ng - 2)) + q);
	...
	diagv.arr.col(0) = c * 2 / dx / dx;
	diagv.arr.col(1) = b * 2 / dx / dx;
	diagv.arr.col(2) = a * 2 / dx / dx;
